...
<template>
  ...
  >
    ...
    >
      ...
      <div
        class="flex items-center gap-2 px-3 py-1.5 rounded-2xl bg-white/95 backdrop-blur-md border border-violet-200/80 shadow-md shadow-violet-500/10 hover:shadow-lg hover:shadow-violet-500/15 hover:border-violet-300 transition-all text-xs"
      >
      ...
      <!-- ĐUÔI BONG BÓNG SUY NGHĨ (2 HẠT TRÒN MÂY NỐI VÀO AVATAR) -->
      <div class="absolute -bottom-1.5 left-3 flex flex-col items-center pointer-events-none">
        <span class="w-2 h-2 rounded-full bg-white border border-violet-200 shadow-2xs -mb-0.5"></span>
        <span class="w-1.5 h-1.5 rounded-full bg-white border border-violet-200 shadow-2xs ml-1.5"></span>
      </div>

      <!-- TOOLTIP / POPOVER MỞ RỘNG KHI HOVER (XEM NHANH) -->
      <Transition name="bubble-pop">
        <div
          v-if="isHovered && isBusy && task && !showDetailModal"
          class="absolute left-0 top-full mt-3 w-72 p-3.5 rounded-2xl bg-white/98 backdrop-blur-xl border border-violet-200/90 shadow-xl shadow-violet-900/10 z-20 text-left pointer-events-none"
        >
          <div class="flex items-center justify-between gap-2 pb-2 border-b border-border/50 mb-2">
            <div class="flex items-center gap-1.5">
              <Sparkles class="w-3.5 h-3.5 text-violet-500" />
              <span class="text-[11px] font-bold text-violet-700 uppercase tracking-wider">
                {{ t('home.partnerStatus.title') }}
              </span>
            </div>
          ...
          <h4 class="text-xs font-bold text-ink leading-snug mb-1">
            {{ task.title }}
          </h4>

          <p v-if="task.content" class="text-[11px] text-ink-muted leading-relaxed line-clamp-3 mb-2">
            {{ task.content }}
          </p>

          <div class="flex items-center justify-between text-[10px] text-ink-faint pt-1.5 border-t border-border/40 font-mono">
            <span class="flex items-center gap-1">
              <Clock class="w-3 h-3 text-violet-400" />
              {{ formatTime(task.start_time) }} &rarr; {{ formatTime(task.end_time) }}
            </span>
            <span class="text-violet-500 font-sans font-medium">
              &rarr;
            </span>
          </div>
        </div>
